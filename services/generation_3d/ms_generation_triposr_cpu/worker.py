...
Descripción:
Worker de generación 3D — Ruta CPU (MS 7.3c). Sin GPU requerida.
Fase 7 — Modernización del Pipeline A con Modelos SOTA 2025-2026.

Genera malla 3D usando TripoSR (Stability AI + VAST, MIT, arXiv:2403.02151).
Única ruta CPU realista: ~30-60s en CPU pura.

Advertencia documentada: topología puede ser sucia (triangulada),
la AI "hallucina" (guesses) lo que se ve en la espalda.
"""
...
celery_app = Celery("ms_generation_triposr_cpu", broker=REDIS_URL, backend=REDIS_URL)
...
def _load_triposr_cpu():
    try:
        from triporsr import TripoSRSR
        print("[*] Cargando TripoSR en modo CPU...")
        model = TripoSRSR.from_pretrained("stabilityai/TripoSR")
        model.set_device("cpu")
        print("[+] TripoSR CPU cargado.")
        return model
    except Exception as e:
        print(f"[!] TripoSR no disponible: {e}")
        return None
...
def generate_placeholder_glb(img: Image.Image) -> bytes:
    ...
    gj = {
        "asset": {"version": "2.0", "generator": "BioRender TripoSR-CPU Placeholder"},
...
@celery_app.task(name="tasks.generate_triposr_cpu", queue="generation_triposr_cpu")
def generate_triposr_cpu(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print(f"[*] Iniciando generación 3D CPU (TripoSR) para Job {job_id}...")
    ...
    try:
        ...
        mesh_key = f"uploads/{job_id}/mesh_triposr_cpu/model.glb"
        ...
        print(f"[PERF_LOG] Task: generate_triposr_cpu | Generación: {gen_dur}ms | Total: {total_dur}ms")
        ...
        print(f"[!] Error en generate_triposr_cpu Job {job_id}: {err}")
        redis_client.set(f"job:status:{job_id}", json.dumps({
