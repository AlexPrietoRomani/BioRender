...
Descripción:
Worker de auto-rigging fallback con Make-It-Animatable (MS 7.4b). CVPR 2025.
Fase 7 — Modernización del Pipeline A con Modelos SOTA 2025-2026.

Soporta poses arbitrarias (no solo T/A-pose), salida FBX Mixamo-compatible en <1s.
Se usa como fallback cuando UniRig falla o la malla es humanoide en pose arbitraria.
"""
...
celery_app = Celery("ms_rigging_mia", broker=REDIS_URL, backend=REDIS_URL)
...
def _load_mia():
    try:
        from make_it_animatable import MIAPipeline
        print("[*] Cargando Make-It-Animatable (CVPR 2025)...")
        pipe = MIAPipeline.from_pretrained("make-it-animatable/mia")
        pipe = pipe.to("cuda")
        print("[+] MIA cargado.")
        return pipe
    except Exception as e:
        print(f"[!] MIA no disponible: {e}")
        return None


def generate_placeholder_rig() -> dict:
    import numpy as np
    weights = np.zeros((1000, 22), dtype=np.float32)
    weights[:, 0] = 1.0
    return {
        "skeleton": MIXAMO_SKELETON,
        "weights": weights.tolist(),
        "vertex_count": 1000,
        "method": "mia_placeholder"
    }


@celery_app.task(name="tasks.rig_mesh_mia", queue="rigging_mia")
def rig_mesh_mia(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print(f"[*] Iniciando auto-rigging MIA (fallback) para Job {job_id}...")
    ...
    try:
        ...
        if model:
            try:
                ...
                print(f"[!] Error en MIA: {e}. Fallback a placeholder.")
                rig_result = generate_placeholder_rig()
        else:
            rig_result = generate_placeholder_rig()
        rig_dur = int((time.perf_counter() - rig_start) * 1000)

        skeleton_key = f"uploads/{job_id}/rigged_mia/skeleton.json"
        weights_key = f"uploads/{job_id}/rigged_mia/weights.json"

        s3_client.put_object(Bucket=BUCKET_NAME, Key=skeleton_key,
                             Body=json.dumps(rig_result["skeleton"], indent=2),
                             ContentType="application/json")
        s3_client.put_object(Bucket=BUCKET_NAME, Key=weights_key,
                             Body=json.dumps(rig_result["weights"]),
                             ContentType="application/json")

        total_dur = int((time.perf_counter() - start_time) * 1000)
        print(f"[PERF_LOG] Task: rig_mesh_mia | Rigging: {rig_dur}ms | Total: {total_dur}ms")

        redis_client.set(f"job:status:{job_id}", json.dumps({
            "status": "processing", "progress": 75, "result_url": "",
            "updated_at": int(time.time()), "stage": "rigging_mia_done"
        }))
        ...
        print(f"[!] Error en rig_mesh_mia Job {job_id}: {err}")
        redis_client.set(f"job:status:{job_id}", json.dumps({
